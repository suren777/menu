...
from menu.db.database import User, RecipeTable
...
async def test_summary_invalid_recipe_id():
    ...
    context = MagicMock()
    context.user_data = {"user": User(id=1, telegram_id=123, premium=False)}
    result = await summary(update, context)
...
@pytest.mark.asyncio
@patch(
    'menu.menu_bot.common.get_recipe_by_id',
    return_value=RecipeTable(id=1, name="Test Recipe", description="Test Description", ingredients="[]", instructions="[]"),
)
async def test_summary_success(mock_get_recipe):
    update = MagicMock()
    query = AsyncMock()
    query.data = "1"
    update.callback_query = query
    context = MagicMock()
    context.user_data = {"user": User(id=1, telegram_id=123, premium=False), "cuisine": "italian", "category": "dessert"}
    result = await summary(update, context)
    query.answer.assert_called_once()
    mock_get_recipe.assert_called_once_with(1)
