...
from menu.crawlers.bbc_good_food.crawler import import_sitemap, import_recipes, crawl_sitemap
...
def test_import_sitemap(mock_sitemap_repo, mock_get_session, mock_get_sitemap):
    ...
    mock_sitemap_repo.url_exists.assert_called_once_with("http://test.com/sitemap1", mock_session)
...
def test_import_recipes(mock_finalise, mock_add_recipe, mock_contains, mock_fetch, mock_request_xml):
    ...
    mock_add_recipe.assert_called_once_with("http://test.com/recipe1", "Test Recipe", {"key": "value"})
...
def test_crawl_sitemap(mock_pool, mock_sitemap_repo, mock_get_session):
    mock_session = MagicMock()
    mock_get_session.return_value.__enter__.return_value = mock_session
    sitemap_entries = [MagicMock(url="http://test.com/sitemap1"), MagicMock(url="http://test.com/sitemap2")]
