...
def test_parse_categories():
    assert parse_categories(["dessert, main", "breakfast"]) == [
        "breakfast",
        "dessert",
        "main",
    ]
...
def test_parse_literal_array():
    ...
    assert (
        parse_literal_array("['one', 'two']", enumerate_list=True) == "1. one\n2. two"
    )
...
def test_create_recipe_filters():
    filters = create_recipe_filters(
        cuisine="italian", category="dessert", ingredient="chocolate"
    )
...
def test_get_recipe_names(mock_get_ro_session):
    ...
    mock_get_ro_session.return_value.__enter__.return_value = mock_session
    assert get_recipe_names(cuisine="italian", category=None) == [
        {"name": "test recipe", "id": 1}
    ]
...
def test_search_recipes_by_ingredients(mock_get_ro_session):
    ...
    mock_session.query.return_value.filter.return_value = [mock_recipe]
    mock_get_ro_session.return_value.__enter__.return_value = mock_session
    assert search_recipes_by_ingredients(["ingredient1"]) == [
        {"name": "test recipe", "id": 1}
    ]
