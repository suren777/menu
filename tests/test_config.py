...
from menu.config import ConfigLoader, ApplicationConfig, load_application_config
...
def test_config_loader_load_from_env(mock_os_environ_get):
    with patch('pathlib.Path.read_text', mock_open(read_data='service: test-service')) as mock_file:
        loader = ConfigLoader()
        config = loader.load()
...
@patch('menu.config.ConfigLoader.load', return_value={"service": "menu", "host": "127.0.0.1", "port": "5000"})
